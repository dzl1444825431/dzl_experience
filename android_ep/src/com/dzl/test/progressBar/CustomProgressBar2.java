...
public class CustomProgressBar2 extends View {
	...
	public CustomProgressBar2(Context context, AttributeSet attrs) {
		...
		final int resID = android.R.anim.linear_interpolator; 
		if (resID > 0) {
	...
	private final int R_id_progress = 0x123;
	private final int R_id_secondaryProgress = 0x124;
		...
		if (drawable instanceof LayerDrawable) {
			...
			for (int i = 0; i < N; i++) {
				...
				outDrawables[i] = tileify(background.getDrawable(i),
						(id == R_id_progress || id == R_id_secondaryProgress));
			}
			...
			//
			// // Ensure the tint and filter are propagated in the correct
			// order.
	...
	/**
	 * Applies a tint to the indeterminate drawable. Does not modify the current
	 ...
	 * indeterminate mode.
	 * <p>
	 * If the drawable is a BitmapDrawable or contains BitmapDrawables, a tiled
	 * copy will be generated for display as a progress bar.
	 *
	 * @param d
	 *            the new drawable
	 * @see #getIndeterminateDrawable()
	...
	public void setProgressDrawable(Drawable d) {
		if (mProgressDrawable != d) {
			...
			// doRefreshProgress(R_id_progress, mProgress, false, false);
			// doRefreshProgress(R_id_secondaryProgress, mSecondaryProgress,
			// false, false);
		}
	}
	...
	private void applyPrimaryProgressTint() {
		if (mProgressTintInfo.mHasProgressTint || mProgressTintInfo.mHasProgressTintMode) {
			final Drawable target = getTintTarget(R_id_progress, true);
			if (target != null) {
	...
	private void applySecondaryProgressTint() {
		if (mProgressTintInfo.mHasSecondaryProgressTint || mProgressTintInfo.mHasSecondaryProgressTintMode) {
			final Drawable target = getTintTarget(R_id_secondaryProgress, false);
			if (target != null) {
	...
	/**
	 * Applies a tint to the progress indicator, if one exists, or to the entire
	 * progress drawable otherwise. Does not modify the current tint mode, which
	 * is {@link PorterDuff.Mode#SRC_IN} by default.
	 * <p>
	 * The progress indicator should be specified as a layer with id
	 * {@link android.R.id#progress} in a {@link LayerDrawable} used as the
	 * progress drawable.
	 * <p>
	...
	/**
	 * Specifies the blending mode used to apply the tint specified by
	 * {@link #setProgressTintList(ColorStateList)} to the progress indicator.
	...
	/**
	 * Applies a tint to the progress background, if one exists. Does not modify
	 * the current tint mode, which is {@link PorterDuff.Mode#SRC_ATOP} by
	 * default.
	 * <p>
	 * The progress background must be specified as a layer with id
	 * {@link android.R.id#background} in a {@link LayerDrawable} used as the
	 * progress drawable.
	 * <p>
	...
	/**
	 * Applies a tint to the secondary progress indicator, if one exists. Does
	 * not modify the current tint mode, which is
	 * {@link PorterDuff.Mode#SRC_ATOP} by default.
	 * <p>
	 * The secondary progress indicator must be specified as a layer with id
	 * {@link android.R.id#secondaryProgress} in a {@link LayerDrawable} used as
	 ...
	 * progress indicator. The default mode is {@link PorterDuff.Mode#SRC_ATOP}.
	 *
	 * @param tintMode
	 *            the blending mode used to apply the tint, may be {@code null}
	 *            to clear tint
	 ...
	 *            whether the base drawable should be returned if the id does
	 *            not exist
	...
	/**
	 * Define the tileable drawable used to draw the progress bar in progress
	 * mode.
	 * <p>
	 * If the drawable is a BitmapDrawable or contains BitmapDrawables, a tiled
	 * copy will be generated for display as a progress bar.
	...
	private class RefreshProgressRunnable implements Runnable {
		public void run() {
			synchronized (CustomProgressBar2.this) {
	...
	private static class RefreshData {
		...
		private static final SynchronizedPool<RefreshData> sPool = new SynchronizedPool<RefreshData>(POOL_MAX);
		...
		public static RefreshData obtain(int id, float progress, boolean fromUser, boolean animate) {
	...
	// private void setDrawableTint(int id, ColorStateList tint, Mode tintMode,
	// boolean fallback) {
	// Drawable layer = null;
	//
	 ...
	 *
	 * @param scale
	 *            Current position/progress between 0 and 1.
	 * @param fromUser
	...
	void setAnimationPosition(float position) {
		...
		refreshProgress(R_id_progress, position, true, true);
	}
	...
	// private synchronized void refreshProgress(int id, float progress, boolean
	// fromUser) {
	...
	protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int dh = 0;
		if (d != null) {
			dw = Math.max(getWidth(), d.getIntrinsicWidth());
			dh = Math.max(getHeight(), d.getIntrinsicHeight());
		}
		updateDrawableState();
