...
class XmlConvertJavaCode2 {
	...
	def convert = {xmlFilePath, parentNodeName = 'ViewGroup', parentNodeValiable = '', level = 0, index = 0, nodeVariableName = ''->
		...
		for (var in variable_list) {
			println "protected ${var};"
		}
		
		println()
		for (var in id_list) {
			var = var.replace('id_', '')
			println "${var} = view.get${captureName(var)}();"
		}
	}
	...
	def textViewMethod = [
		...
		'maxLength':'setFilters(new InputFilter[] { new InputFilter.LengthFilter',
		'lines':'setLines',
		'text':'setText',
	...
	def recursion = {node, parentNodeName, parentVariableName, level, nodeVaribleNameOld = '' ->
		...
		node.attributes().each {
			...
			if (attr == 'id') {
//				println (nodeVariableName + "111111111=============${val.substring(val.indexOf('/'))}")
				nodeVariableName = val.substring(val.indexOf('/') + 1);
...
//					val = "(int)res.getDimension(R.dimen." + val.substring(ind) + ")"
					val = "translate(context, " + val.substring(val.indexOf('_') + 1, val.indexOf('px')) + ")"
...
//					val = "R.id." + val.substring(ind)
					...
					if(val.startsWith("@+")){
						id_list.add("id_${val.substring(ind)}")
					}
					val = "id_" + val.substring(ind)
				}
				...
				if (val == '0dp') {
					...
					val = "DensityUtil.dip2px(getDensity(context) , ${val.replace('dp','')})"
				}
			}else if (val.endsWith('dip')){
				if (val == '0dip') {
					val = '0';
				} else {
					val = "DensityUtil.dip2px(getDensity(context) , ${val.replace('dip','')})"
				}
			}else if (val.endsWith('sp')){
				if (val == '0sp') {
					val = '0';
				} else {
					val = "DensityUtil.sp2px(getDensity(context) , ${val.replace('sp','')})"
				}
		...
		println "${is_member ?'':(nodeName + ' ')}${nodeVariableName} = new ${nodeName}(context); // level = ${level}"
		...
		if (nodeName == 'TextView' || nodeName == 'CheckBox' || nodeName == 'Button' || nodeName.contains('EditText')) {
				...
				textViewMethod.each {
					if (attrubite.key == it.key) {
						...
						if (it.key == 'gravity') {
							...
							val = val.replace('translate','translateToTextSize')
						}else if(it.key == 'maxLength') {
							val = "${val})}"
						}
